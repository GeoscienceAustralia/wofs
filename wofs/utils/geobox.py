...
class GriddedGeoBox(object):
    ...
    def as_json(self):
        d = {'origin': self.origin, \
            ...
            'crs' : self.crs.ExportToWkt()}
        return json.dumps(d)

    @staticmethod
    def from_json(json_string):
        d = json.loads(json_string)
        return GriddedGeoBox( \
            origin=tuple(d['origin']), \
            shape=tuple(d['shape']), \
            pixelsize=tuple(d['pixelsize']), \
            crs=bytes(d['crs']))
    ...
    def get_window_geobox(self, window):
        """
        Return the GriddedGeoBox describing the windowed area of this
        GriddedGeoBox
        ...
        new_origin = (col_start, row_start) * self.affine
        return GriddedGeoBox(shape=new_shape, origin=new_origin, \
            pixelsize=self.pixelsize, crs=self.crs)
